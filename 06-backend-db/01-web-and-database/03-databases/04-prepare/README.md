...
```bash
go run ./06-backend-db/01-web-and-database/03-databases/04-prepare
```
...
> [!TIP]
> Prepared statements make individual queries faster. But what if you need a group of queries to succeed or fail as a single unit (like a bank transfer)? In [Lesson 5: Transactions](../05-transactions/README.md), you will learn how to use `db.Begin` to ensure data integrity.

## Next Step

Next: `DB.5` -> [`06-backend-db/01-web-and-database/03-databases/05-transactions`](../05-transactions/README.md)
