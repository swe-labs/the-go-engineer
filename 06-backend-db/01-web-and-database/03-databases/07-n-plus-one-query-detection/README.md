...
```bash
go run ./06-backend-db/01-web-and-database/03-databases/07-n-plus-one-query-detection
```
...
> [!TIP]
> You've optimized your queries. But what happens if the database is still slow or locked? You can't let your server wait forever. In [Lesson 8: Query Timeouts via Context](../08-query-timeouts-via-context/README.md), you will learn how to use `context.Context` to set strict time limits on every database operation.

## Next Step

Next: `DB.8` -> [`06-backend-db/01-web-and-database/03-databases/08-query-timeouts-via-context`](../08-query-timeouts-via-context/README.md)
