...
| --- | --- | --- | --- | --- |
| **DOCKER.1** | Lesson | [Docker Basics](./01-docker-basics) | Master the `Dockerfile` for Go. | entry |
| **DOCKER.2** | Lesson | [Multi-stage Builds](./02-multi-stage-builds) | Build in one image, run in a minimal one. | **DOCKER.1** |
| **DOCKER.3** | Lesson | [Docker Compose](./03-docker-compose) | Orchestrate a Service + Database locally. | **DOCKER.2** |
| **DEPLOY.1** | Lesson | [CI/CD Pipelines](./04-cicd-pipelines) | Automate testing and building on every push. | **DOCKER.3** |
| **DEPLOY.2** | Lesson | [Blue/Green & Rollback](./05-blue-green-and-rollback) | Learn zero-downtime deployment patterns. | **DEPLOY.1** |
| **DEPLOY.3** | Exercise | [Dockerised Service](./06-dockerised-service) | Containerize a complete Go API with DB. | **DOCKER/DEPLOY** |
