...
# $env:APP_PORT=8080; go run ./10-production/04-configuration/05-config-validation-on-boot

# Run with INVALID config to see the fail-fast behavior
# $env:APP_PORT=-1; go run ./10-production/04-configuration/05-config-validation-on-boot
```
...
Next: `OPS.1` -> [`10-production/05-observability/01-metrics-basics`](../../05-observability/01-metrics-basics/README.md)
