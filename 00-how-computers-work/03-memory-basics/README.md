...
> [!TIP]
> At high scale, excessive heap allocation creates Garbage Collector pressure. You can learn how to profile and optimize memory layout in [PR.6 Memory Layout](../../08-quality-test/01-quality-and-performance/01-profiling/06-memory-layout/README.md).

> [!NOTE]
> Because the heap is shared memory, concurrent access to heap objects requires synchronization, which we cover in [SY.1 Mutex & RWMutex](../../07-concurrency/01-concurrency/03-sync-primitives/01-mutex-and-rwmutex/README.md).

## Run Instructions

```bash
go run ./00-how-computers-work/03-memory-basics
```
...
3. Run `go build -gcflags='-m' ./00-how-computers-work/03-memory-basics` and read the compiler's escape-analysis hints in the terminal.
...
Next: `HC.4` -> [`00-how-computers-work/04-terminal-confidence`](../04-terminal-confidence/README.md)
