# Lesson: Source code, executable, and process

## Mission

Learn the difference between text you edit, the executable artifact produced from it, and the live process running on the machine.
...
- core-01-01 (What is a program)

Understand a program as a set of instructions the computer executes.

## Mental Model

Source code is a blueprint, an executable is the built machine, and a process is the machine turned on and doing work.
...
The compiler turns source into machine instructions. The operating system starts a process by loading an executable, assigning memory, and scheduling CPU time.
...
```bash
go run ./curriculum/modules/01-computers-terminal-git-web/lessons/02-source-code-executable-and-process
go test ./curriculum/modules/01-computers-terminal-git-web/lessons/02-source-code-executable-and-process
```
...
Explain what changes when code moves from source to executable to process.
...
|---|---|---|
| Changing source code and assuming a running process changed automatically. | The visible surface hides an important machine boundary. | Rebuild and restart when the source change must affect a running process. |
...
Build pipelines, deployment systems, and incident debugging all depend on knowing whether a bug lives in source, build artifact, runtime configuration, or the live process.
...
- explain `Source code, executable, and process` in your own words
- run the example
- run the test
...
```text
core-01-03 — ../03-files-bytes-directories-and-paths/README.md
```
