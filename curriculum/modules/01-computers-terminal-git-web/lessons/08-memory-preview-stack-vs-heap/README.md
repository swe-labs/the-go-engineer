# Lesson: Memory preview: stack vs heap

## Mission

Build a beginner-safe preview of memory as storage that programs use while running.
...
Understand process lifecycle, scheduling, and standard streams.

## Mental Model

The stack is like a neat pile of call frames. The heap is like a larger shared storage room for values that need flexible lifetime.
...
Go decides where values live using escape analysis. The garbage collector later reclaims heap values that are no longer reachable.
...
```bash
go run ./curriculum/modules/01-computers-terminal-git-web/lessons/08-memory-preview-stack-vs-heap
go test ./curriculum/modules/01-computers-terminal-git-web/lessons/08-memory-preview-stack-vs-heap
```
...
Explain why memory matters before needing to master allocation details.
...
|---|---|---|
| Thinking stack means fast and heap means bad. | The visible surface hides an important machine boundary. | Treat stack and heap as lifetime tools; measure before optimizing. |
...
Memory pressure, leaks, and allocation-heavy code can create latency and reliability problems.
...
- explain `Memory preview: stack vs heap` in your own words
- run the example
- run the test
...
```text
core-01-09 — ../09-git-mental-model/README.md
```
