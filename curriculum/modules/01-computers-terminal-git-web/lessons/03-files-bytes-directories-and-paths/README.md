# Lesson: Files, bytes, directories, and paths

## Mission

Understand files as named byte sequences and paths as addresses used to locate them.
...
- core-01-02 (Source code, executable, and process)

Understand how source code becomes a running process.

## Mental Model

A filesystem is a labeled warehouse. Directories are shelves, files are boxes, and paths are written directions to a box.
...
The operating system stores bytes and metadata. Programs ask the OS to open, read, write, and close files using paths.
...
```bash
go run ./curriculum/modules/01-computers-terminal-git-web/lessons/03-files-bytes-directories-and-paths
go test ./curriculum/modules/01-computers-terminal-git-web/lessons/03-files-bytes-directories-and-paths
```
...
Classify a path as absolute or relative and explain where the program will look.
...
|---|---|---|
| Assuming a relative path starts from the file's directory. | The visible surface hides an important machine boundary. | Remember that relative paths are resolved from the current working directory of the process. |
...
File path mistakes cause missing config, failed migrations, broken assets, and data loss during deployments.
...
- explain `Files, bytes, directories, and paths` in your own words
- run the example
- run the test
...
```text
core-01-04 — ../04-terminal-basics/README.md
```
