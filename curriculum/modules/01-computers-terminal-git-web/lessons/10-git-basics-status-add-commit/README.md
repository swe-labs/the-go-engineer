# Lesson: Git basics: status, add, commit

## Mission

Learn the safe beginner Git loop: inspect, stage, commit, inspect again.
...
- core-01-09 (Git mental model)

Understand Git as a snapshot history database.

## Mental Model

`git status` is the dashboard, `git add` prepares a snapshot, and `git commit` records that snapshot.
...
Git compares the working tree, index, and last commit to show what changed and what is staged.
...
```bash
go run ./curriculum/modules/01-computers-terminal-git-web/lessons/10-git-basics-status-add-commit
go test ./curriculum/modules/01-computers-terminal-git-web/lessons/10-git-basics-status-add-commit
```
...
Practice the inspect-stage-commit loop on a harmless file.
...
|---|---|---|
| Running `git add .` without checking what changed. | The visible surface hides an important machine boundary. | Run `git status` and `git diff` before staging. |
...
Small, focused commits make code review, rollback, and debugging much easier.
...
- explain `Git basics: status, add, commit` in your own words
- run the example
- run the test
...
```text
core-01-11 — ../11-branching-and-merging/README.md
```
