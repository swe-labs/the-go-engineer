...
Become comfortable using the terminal as a text interface for navigating folders, running programs, and reading output.
...
- core-01-03 (Files, bytes, directories, and paths)

Understand files as named byte sequences and paths as addresses.

## Mental Model

The terminal is a conversation with the computer: you type a command, the shell interprets it, and the program writes back.
...
A shell parses your command line, resolves the executable, passes arguments and environment variables, then waits for an exit code.
...
```bash
go run ./curriculum/modules/01-computers-terminal-git-web/lessons/04-terminal-basics
go test ./curriculum/modules/01-computers-terminal-git-web/lessons/04-terminal-basics
```
...
Use `pwd`, `ls`, and `cd` to navigate to this lesson and run the example.
...
|---|---|---|
| Typing commands from the wrong directory and blaming the tool. | The visible surface hides an important machine boundary. | Run `pwd` first and verify the path before running a command. |
...
Servers, containers, CI jobs, and deployment systems are operated through command-line interfaces.
...
- explain `Terminal basics` in your own words
- run the example
- run the test
...
```text
core-01-05 — ../05-environment-variables/README.md
```
