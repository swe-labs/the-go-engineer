# Lesson: How to use this repository

## Mission

Learn how the repository is organized and how metadata connects to learner-facing files.
...
Think of the repository as a city. `metadata/` is the map, `curriculum/` is the city you walk through, `tools/` are the inspectors, `docs/` are the laws, and `dist/` is the export package.
...
The validator reads JSON metadata, follows paths into `curriculum/`, checks files, and fails when the graph says something exists but the filesystem disagrees.
...
```bash
go run ./curriculum/modules/00-orientation/lessons/01-how-to-use-this-repository
...
Find three lesson IDs in metadata, locate their README paths, and explain the relationship between the JSON entry and the folder on disk.
...
```text
core-00-02 — ../02-what-zero-magic-means/README.md
```
