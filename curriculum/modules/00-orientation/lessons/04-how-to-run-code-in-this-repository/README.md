# Lesson: How to run code in this repository

## Mission

Learn the difference between reading code, running code, testing code, and validating curriculum structure.
...
Running code asks, 'What does this program do?' Testing asks, 'Does it keep doing the right thing?' Validation asks, 'Does this curriculum item meet the contract?'
...
`go run` compiles and executes a package. `go test` compiles test files and runs test functions. Validators inspect metadata and files.
...
```bash
go run ./curriculum/modules/00-orientation/lessons/04-how-to-run-code-in-this-repository
...
Run the lesson, run the test, then explain the difference between the two outputs.
...
```text
core-00-05 — ../05-how-starter-folders-work/README.md
```
