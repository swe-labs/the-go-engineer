# Lesson: Reading test failures

## Mission

Learn to use test failure output to identify expected behavior, actual behavior, and the smallest broken assumption.
...
- core-02-11 (Reading runtime errors)

Understand panic tracebacks, stack frames, and how to locate the exact line that caused a panic.

## Mental Model

A failing test is not a punishment. It is a precise disagreement between expectation and reality.
...
The test binary reports which test failed, which assertion failed, and any messages written with `t.Fatal`, `t.Fatalf`, `t.Error`, or `t.Errorf`.
...
```bash
go run ./curriculum/modules/02-go-setup-tooling/lessons/12-reading-test-failures
go test ./curriculum/modules/02-go-setup-tooling/lessons/12-reading-test-failures
```
...
Read a test failure and identify test name, expected value, actual value, and likely source of the bug.
...
|---|---|---|
| Fixing code randomly until tests pass. | The command hides more than one stage. | Read the failing assertion and explain what behavior the test is protecting. |
...
High-quality test failures shorten incident response and code review because they explain what broke.
...
- explain `Reading test failures` in your own words
- run the example
- run the test
...
```text
core-02-13 — ../13-go-module-root-basics/README.md
```
