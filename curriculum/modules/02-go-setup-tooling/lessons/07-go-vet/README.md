...
Use `go vet` as a static analyzer that catches suspicious code patterns tests may miss.
...
Understand that `gofmt` enforces canonical Go formatting and be comfortable running it.

## Mental Model

`go vet` is a cautious reviewer that looks for code smells with likely bugs.
...
`go vet` analyzes Go syntax and type information to detect risky constructs, such as malformed format strings or unreachable patterns.
...
```bash
go run ./curriculum/modules/02-go-setup-tooling/lessons/07-go-vet
go test ./curriculum/modules/02-go-setup-tooling/lessons/07-go-vet
```
...
Run `go vet` and explain why a clean result is useful but not a replacement for tests.
...
|---|---|---|
| Thinking `go vet` proves the program is correct. | The command hides more than one stage. | Use vet with tests, review, and runtime validation; each catches different failure modes. |
...
CI often runs vet to catch bugs before they reach review or deployment.
...
- explain `go vet` in your own words
- run the example
- run the test
...
```text
core-02-08 — ../08-go-doc/README.md
```
