...
Use `gofmt` to make Go formatting automatic and non-negotiable.
...
Understand how to run tests with `go test` and interpret PASS/FAIL output.

## Mental Model

`gofmt` removes style debates by giving every Go file the same shape.
...
`gofmt` parses Go source into syntax and prints it back in the canonical format.
...
```bash
go run ./curriculum/modules/02-go-setup-tooling/lessons/06-gofmt
go test ./curriculum/modules/02-go-setup-tooling/lessons/06-gofmt
```
...
Run `gofmt -w` on a file and explain why formatting should not be manual.
...
|---|---|---|
| Manually formatting Go code and creating inconsistent diffs. | The command hides more than one stage. | Run `gofmt` or configure your editor to format on save. |
...
Consistent formatting keeps code reviews focused on behavior and design rather than whitespace.
...
- explain `gofmt` in your own words
- run the example
- run the test
...
```text
core-02-07 — ../07-go-vet/README.md
```
