...
// Account.Deposit (Method): modifies account state by adding funds; uses a pointer receiver for persistence.
...
// Account.Withdraw (Method): implements standard fund removal with strict balance checks.
...
// Account.GetBalance (Method): provides read-only access to the internal balance state.
func (acc *Account) GetBalance() float64 {
	return acc.Balance
}

// String (Method) implements the fmt.Stringer interface for domain-specific summary output.
// Account.String (Method): implements the fmt.Stringer interface for domain-specific summary output.
...
// SavingsAccount.AddInterest (Method): implements interest calculation logic and utilizes the promoted Deposit method.
...
// OverdraftAccount.Withdraw (Method): shadows the base Account.Withdraw method to implement specialized overdraft logic.
