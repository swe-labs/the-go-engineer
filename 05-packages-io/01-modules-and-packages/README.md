...
| --- | --- | --- | --- | --- |
| `MP.1` | Lesson | [module basics](./01-module-basics) | Explains what `go.mod` and `go.sum` actually do. | entry |
| `MP.2` | Lesson | [managing deps](./02-managing-deps) | Shows how to add, inspect, trim, and explain dependencies. | `MP.1` |
| `MP.3` | Exercise | [versioning workshop](./03-versioning) | Combines semantic versioning, `/v2` imports, and `replace`. | `MP.1`, `MP.2` |
| `MP.4` | Stretch Lesson | [build tags](./04-build-tags) | Adds conditional compilation once the module workflow is stable. | `MP.1`, `MP.2` |
