...
```bash
go run ./05-packages-io/02-io-and-cli/01-cli-tools/04-file-organizer
```

Run on a specific directory:
```bash
go run ./05-packages-io/02-io-and-cli/01-cli-tools/04-file-organizer --dir=./my-folder
```

Perform a safe preview (Dry Run):
```bash
go run ./05-packages-io/02-io-and-cli/01-cli-tools/04-file-organizer --dir=./my-folder --dry-run
```
...
- Use `go run ./05-packages-io/02-io-and-cli/01-cli-tools/04-file-organizer`.
- Starter path: `05-packages-io/02-io-and-cli/01-cli-tools/04-file-organizer/_starter`.
...
> [!TIP]
> You have mastered the art of CLI interaction and basic file manipulation. Now we need to look at how to handle more complex data formats. In [Lesson 1: Marshalling](../../02-encoding/01-marshalling/README.md), you will learn how to turn Go structs into JSON and XML strings.

## Next Step

Next: `EN.1` -> [`05-packages-io/02-io-and-cli/02-encoding/01-marshalling`](../../02-encoding/01-marshalling/README.md)
