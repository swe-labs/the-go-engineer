...
```bash
go run ./05-packages-io/02-io-and-cli/01-cli-tools/03-subcommands
```

Try different subcommands:
```bash
go run ./05-packages-io/02-io-and-cli/01-cli-tools/03-subcommands greet -name="Gopher" -loud
go run ./05-packages-io/02-io-and-cli/01-cli-tools/03-subcommands calc -a=10 -b=5 -op=sub
go run ./05-packages-io/02-io-and-cli/01-cli-tools/03-subcommands version
```

Check subcommand-specific help:
```bash
go run ./05-packages-io/02-io-and-cli/01-cli-tools/03-subcommands greet -help
```
...
> [!TIP]
> You now know how to build a sophisticated CLI interface. It's time to put these skills to work. In [Lesson 4: File Organizer Project](../04-file-organizer/README.md), you will build a real-world tool that organizes files into directories based on their extensions.

## Next Step

Next: `CL.4` -> [`05-packages-io/02-io-and-cli/01-cli-tools/04-file-organizer`](../04-file-organizer/README.md)
