...
```bash
go run ./05-packages-io/02-io-and-cli/01-cli-tools/02-flags
```

Try passing typed flags:
```bash
go run ./05-packages-io/02-io-and-cli/01-cli-tools/02-flags -name="Gopher" -count=5 -verbose
```

Check the auto-generated help:
```bash
go run ./05-packages-io/02-io-and-cli/01-cli-tools/02-flags -help
```
...
> [!TIP]
> Flags are great for single-purpose tools. But what if your app has multiple distinct modes (like `git add` and `git commit`)? In [Lesson 3: Subcommands](../03-subcommands/README.md), you will learn how to build multi-command CLI applications.

## Next Step

Next: `CL.3` -> [`05-packages-io/02-io-and-cli/01-cli-tools/03-subcommands`](../03-subcommands/README.md)
