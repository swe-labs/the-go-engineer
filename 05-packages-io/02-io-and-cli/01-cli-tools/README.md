...
| --- | --- | --- | --- | --- |
| `CL.1` | Lesson | [args](./01-args) | Introduces raw command-line arguments, environment variables, and exit codes. | entry |
| `CL.2` | Lesson | [flags](./02-flags) | Shows typed option parsing with the `flag` package. | `CL.1` |
| `CL.3` | Lesson | [subcommands](./03-subcommands) | Builds multi-command CLIs without third-party frameworks. | `CL.1`, `CL.2` |
| `CL.4` | Exercise | [file organizer](./04-file-organizer) | Combines flags, directory reading, and safe filesystem changes in one milestone. | `CL.1`, `CL.2`, `CL.3` |
...
then the CLI part of Stage 05 is doing its job.

## Next Step

After `CL.4`, continue to the [Encoding track](../02-encoding) or back to the
