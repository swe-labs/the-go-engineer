...
| --- | --- | --- | --- | --- |
| `CO.1` | Lesson | [composition](./05-composition-and-embedding/1-composition) | Introduces has-a modeling with named fields so reuse stays explicit. | entry |
| `CO.2` | Lesson | [embedding](./05-composition-and-embedding/2-embedding) | Adds promoted fields and methods while clarifying that embedding is still composition. | `CO.1` |
| `CO.3` | Exercise | [bank account project](./05-composition-and-embedding/3-bank-account) | Combines composition, embedding, promoted methods, and method shadowing in one milestone. | `CO.1`, `CO.2` |
...
- `CO.1` and `CO.2` stay in `05-composition/05-composition-and-embedding`
- `CO.3` stays at `05-composition/05-composition-and-embedding/3-bank-account`, now treated as
...
After `CO.3`, continue to [Section 07: Strings and Text](../06-strings-and-text).
