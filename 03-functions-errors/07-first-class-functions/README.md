...
> [!NOTE]
> In [FE.6 Orchestration](../06-orchestration/README.md), you hardcoded the exact helpers that `processCart` called. First-class functions let you pass the behavior *into* the function dynamically, rather than hardcoding it.
...
```bash
go run ./03-functions-errors/07-first-class-functions
```
...
> [!TIP]
> You now know how to assign anonymous functions to variables. But what happens if that anonymous function references variables from the surrounding code? In [FE.9 Closures Mechanics](../08-closures-mechanics/README.md), you will learn how functions can capture and remember state.
...
Next: `FE.9` -> [`03-functions-errors/08-closures-mechanics`](../08-closures-mechanics/README.md)
