...
> [!NOTE]
> In [FE.3 Multiple Return Values](../03-multiple-return-values/README.md), you learned how to return more than one result. Returning an error is simply the most common and important application of that feature in Go.
...
```bash
go run ./03-functions-errors/04-errors-as-values
```
...
> [!TIP]
> Now you know how to return errors when an operation fails. But how do you prevent bad data from causing operations to fail in the first place? In [FE.5 Validation](../05-validation/README.md), you will learn how to check inputs at the function boundary before performing expensive or dangerous work.
...
Next: `FE.5` -> [`03-functions-errors/05-validation`](../05-validation/README.md)
