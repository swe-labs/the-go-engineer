...
> [!NOTE]
> In [FE.2 Parameters and Returns](../02-parameters-and-returns/README.md), you learned how to return a single computed result. Multiple return values simply extend that syntax, allowing functions to remain explicit and honest about all facets of an outcome.
...
```bash
go run ./03-functions-errors/03-multiple-return-values
```
...
> [!TIP]
> You now know how to return two values. The most common use case for this in Go is returning a result *and* an error object. In [FE.4 Errors As Values](../04-errors-as-values/README.md), you will learn how Go handles failures without using exceptions.
...
Next: `FE.4` -> [`03-functions-errors/04-errors-as-values`](../04-errors-as-values/README.md)
