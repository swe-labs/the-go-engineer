...
This exercise is the live Section 10 milestone.
...
```bash
go run ./10-web-and-database/databases/6-repository
```

Run the tests:

```bash
go test ./10-web-and-database/databases/6-repository
```

Run the starter:

```bash
go run ./10-web-and-database/databases/6-repository/_starter
```
...
After you complete this milestone, continue to [Section 11: Concurrency](../../11-concurrency) or
explore the other Section 10 legacy reference surfaces if you need more web/database examples first.
