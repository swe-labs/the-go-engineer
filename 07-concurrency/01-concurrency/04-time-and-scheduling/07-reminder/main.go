...
//
// RUN:
//   go run ./07-concurrency/01-concurrency/04-time-and-scheduling/07-reminder
//
// KEY TAKEAWAY:
//   - Build a small reminder app that counts down with a ticker and fires a one-shot reminder with `time.AfterFunc`. This exercise is the Time and Schedu...
// ============================================================================
...
//
// USAGE: go run ./07-concurrency/01-concurrency/04-time-and-scheduling/07-reminder <seconds> <message>
// EXAMPLE: go run ./07-concurrency/01-concurrency/04-time-and-scheduling/07-reminder 5 "Take a break!"
...
func runReminder(seconds int, message string) {
	...
	for {
		...
		case <-done:
			...
			fmt.Println()
			fmt.Println("NEXT UP: CP.1 -> 07-concurrency/02-concurrency-patterns/01-errgroup")
			return
		}
	}
}
