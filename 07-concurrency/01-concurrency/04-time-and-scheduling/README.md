...
| --- | --- | --- | --- | --- |
| `TM.1` | Lesson | [time basics](./01-time) | Introduces `time.Time`, `time.Duration`, and elapsed-time calculations. | entry |
| `TM.2` | Lesson | [formatting](./02-formatting) | Teaches parsing and formatting with Go's reference-time model. | `TM.1` |
| `TM.3` | Lesson | [timers and tickers](./03-timer-and-ticker) | Explains one-shot and repeating timed events plus cleanup rules. | `TM.1`, `TM.2` |
| `TM.7` | Exercise | [console reminder](./07-reminder) | Combines timers, tickers, and command-line input in one runnable milestone. | `TM.1`, `TM.2`, `TM.3` |
