...
//
// RUN: go run ./07-concurrency/01-concurrency/03-sync-primitives/04-race-conditions
// ============================================================================
...
func main() {
	...
	fmt.Println("1??  Unsafe (no protection):")
	for i := 0; i < 3; i++ {
	...
	fmt.Println("2??  Mutex (sync.Mutex):")
	start := time.Now()
	result := mutexCounter()
	...
	fmt.Println("3??  Atomic (sync/atomic):")
	start = time.Now()
	...
	fmt.Println("\n---------------------------------------------------")
	fmt.Println("NEXT UP: SY.5 -> 07-concurrency/01-concurrency/03-sync-primitives/05-goroutine-leaks")
