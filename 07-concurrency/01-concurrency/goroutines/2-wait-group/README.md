...
2. Move `wg.Add(1)` to the first line _inside_ the `checkService` function. Why is this a race condition? (Hint: The main loop might finish and call `Wait()` before any of the goroutines actually start and call `Add()`).
