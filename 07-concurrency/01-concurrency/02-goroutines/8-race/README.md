...
Run normally first:
```bash
go run ./07-concurrency/01-concurrency/02-goroutines/8-race
```

Then run with the **Race Detector**:
```bash
go run -race ./07-concurrency/01-concurrency/02-goroutines/8-race
```
...
We've seen the basics of Mutex and Atomic. Now let's dive deep into the full toolbox of synchronization primitives. Continue to [GC.9 Select Deep Dive](../9-select-deep-dive/README.md).
