...
This live v2 slice keeps the current `10-concurrency` layout intact while upgrading the learner-facing flow:
...
- `GC.1` through `GC.7` stay in `10-concurrency/goroutines/*`
- `CT.1` through `CT.5` stay in `10-concurrency/context/*`
- `TM.1`, `TM.2`, `TM.3`, and `TM.7` stay in `10-concurrency/time-and-scheduling/*`
...
[Section 12: Concurrency Patterns](../11-concurrency-patterns).
