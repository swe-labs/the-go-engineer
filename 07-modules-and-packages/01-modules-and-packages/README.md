...
then you are ready to move into file I/O and CLI tooling in Stage 05.

## Pilot Role In V2

This live v2 slice keeps the current Stage 05 paths and `MP.*` ids stable while upgrading the
...
This pilot intentionally avoids breaking the current Stage 05 filesystem layout.

- `MP.1` stays at `07-modules-and-packages/1-module-basics`
- `MP.2` stays at `07-modules-and-packages/2-managing-deps`
- `MP.3` stays at `07-modules-and-packages/3-versioning`, now treated as the section milestone
  surface with a starter and tests
- `MP.4` stays at `07-modules-and-packages/4-build-tags` as an optional stretch lesson
...
After `MP.3`, continue to [Stage 05: I/O and CLI Tools](../07-modules-and-packages/02-io-and-cli).
In the beta shell, that keeps you inside
[05 Packages & IO](../docs/stages/05-packages-io.md).
